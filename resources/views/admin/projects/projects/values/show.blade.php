...
@section('content')
    <div class="d-flex flex-row mb-3">
        <a href="{{ route('admin.projects.values.edit', ['product' => $product, 'characteristic' => $characteristic]) }}" class="btn btn-primary mr-1">{{ trans('adminlte.edit') }}</a>
        <form method="POST" action="{{ route('admin.projects.values.destroy', ['project' => $project, 'characteristic' => $characteristic]) }}" class="mr-1">
            @csrf
            @method('DELETE')
