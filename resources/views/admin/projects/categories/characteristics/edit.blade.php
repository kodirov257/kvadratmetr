...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.projects.categories.characteristics.update', [$category, $characteristic]) }}">
        @csrf
        @method('PUT')

        <div class="form-group">
            <label for="name" class="col-form-label">Name</label>
            <input id="name" class="form-control{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name', $characteristic->name) }}" required>
            @if ($errors->has('name'))
        ...
        <div class="form-group">
            ...
            <input id="sort" type="text" class="form-control{{ $errors->has('sort') ? ' is-invalid' : '' }}" name="sort" value="{{ old('sort', $characteristic->sort) }}" required>
            @if ($errors->has('sort'))
        ...
        <div class="form-group">
            ...
            <select id="type" class="form-control{{ $errors->has('type') ? ' is-invalid' : '' }}" name="type">
                @foreach ($types as $type => $label)
                    <option value="{{ $type }}"{{ $type == old('type', $characteristic->type) ? ' selected' : '' }}>{{ $label }}</option>
                @endforeach;
            </select>
            @if ($errors->has('type'))
        ...
        <div class="form-group">
            ...
            <div class="checkbox">
                <label>
                    <input type="checkbox" name="required" {{ old('required', $characteristic->required) ? 'checked' : '' }}> Required
                </label>
            </div>
