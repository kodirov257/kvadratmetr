<?php

return [

    'full_name'                   => 'Полное имя',
    'email'                       => 'Почта',
    'phone'                       => 'Номер телефона',
    'password'                    => 'Пароль',
    'retype_password'             => 'Подтверждение пароля',
    'remember_me'                 => 'Запомнить меня',
    'register'                    => 'Регистрация',
    'register_a_new_membership'   => 'Регистрация нового пользователя',
    'i_forgot_my_password'        => 'Восстановление пароля',
    'i_already_have_a_membership' => 'Я уже зарегистрирован',
    'sign_in'                     => 'Вход',
    'log_out'                     => 'Выход',
    'toggle_navigation'           => 'Переключить навигацию',
    'login_message'               => 'Вход в систему',
    'register_message'            => 'Регистрация нового пользователя',
    'password_reset_message'      => 'Восстановление пароля',
    'reset_password'              => 'Восстановление пароля',
    'send_password_reset_link'    => 'Отправить ссылку для восстановления пароля',
    'change_password'             => 'Изменить пароль',
    'verify_message'              => 'Your account needs a verification',
    'verify_email_sent'           => 'A fresh verification link has been sent to your email address.',
    'verify_check_your_email'     => 'Before proceeding, please check your email for a verification link.',
    'verify_if_not_recieved'      => 'If you did not receive the email',
    'verify_request_another'      => 'click here to request another',
    'confirm_password_message'    => 'Please, confirm your password to continue.',
    'search'                      => 'Поиск',
    'home'                        => 'Главный',
    'is_published'                => 'Опубликован',
    'common'                      => 'Общие',
    'longitude'                   => 'Долгота',
    'latitude'                    => 'Широта',
    'content'                     => 'Контент',
    'landmark'                     => 'Ориентир',
    'address' => 'Адрес',
    'name' => 'Имя',
    'about' => 'Описание',
    'comment' => 'Комментарий',
    'title' => 'Заголовок',
    'region' => 'Область',
    'type' => 'Тип',
    'required' => 'Требуется',





    'create' => 'Добавить',
    'save' => 'Добавить',
    'upload' => 'Загрузить',
    'edit' => 'Редактировать',
    'delete' => 'Удалить',
    'delete_confirmation_message' => 'Вы уверены?',
    'main' => 'Основной',
    'yes' => 'Да',
    'no' => 'Нет',
    'draft' => 'Черновой вариант',
    'publish' => 'Опубликовать',
    'published' => 'Опубликованный',
    'send_to_moderation' => 'Отправить на модерацию',
    'moderate' => 'Модерировать',
    'activate' => 'Активировать',
    'close' => 'Закрыть',
    'inactive' => 'Неактивный',
    'active' => 'Активный',

    'users' => 'Пользователи',
    'projects' => 'Проекты',

    'status' => 'Состояние',
    'price' => 'Цена',
    'additional' => 'Дополнительная информация',
    'owner' => 'Владелец',
    'reject_reason' => 'Причина отказа',
    'created_at' => 'Создано в',
    'updated_at' => 'Обновлено в',
    'others' => 'Дополнительная информация',

    'area' => 'Площадь',
    'area_unit' => 'Единицы площади',
    'rooms' => 'Комната',
    'bathroom' => 'Санузел',
    'variants' => 'Варианты',

    'user' => [
        'name' => 'Имя Пользователя',
        'add' => 'Добавить пользователя',
        'verify' => 'Подтверждение пользователя',
        'profile' => 'Профиль',
        'role' => 'Роль',
        'waiting' => 'В ожидании',
        'active' => 'Активный',
        'role_user' => 'Пользователь',
        'role_moderator' => 'Модератор',
        'role_administrator' => 'Администратор',
    ],
    'developer' => [
        'create' => 'Создать разработчика',
        'name' => 'Разработчик'
    ],
    'contact' => [
        'title' => 'Контакты',
        'number' => 'Контактный телефон',
        'call_center' => 'Номер колл-центра',
        'website' => 'Веб-сайт',
    ],
    'project' => [
        'add' => 'Добавить проект',
        'name' => 'Название проекта',
        'add_photos' => 'Добавить фотографии',

    ],
    'sale-offices' => [
        'name'=> 'Офисы продаж',
        'add' => 'Добавить офисы продаж'
    ],
    'value'=>[
        'add' => 'Добавить значение',
        'name' => 'Имя значения',
    ],
    'facilities'=>[
        'add' => 'Добавить объекты'
    ],
    'plan'=>[
        'add' => 'Добавить план',
        'name' => 'Название плана',
        'area' => 'Площадь',
        'area_unit' => 'Единицы площади',
        'rooms' => 'Комната',
        'bathroom' => 'Санузел',

    ],
    'characteristic' => [
        'name' => 'Характеристики',
        'add' => 'Добавить Характеристики',
        'is_range' => 'Диапазон',
    ],
    'characteristics' => [
        'name' => 'Характеристики',
        'add' => 'Добавить Характеристики',
        'is_range' => 'Диапазон',
    ],
    'photo' => [
        'plural' => 'Фото',
        'add' => 'Добавить фотографии',
    ],
    'facility'=> [
        'name' => 'Средство',
        'add' => 'Добавить Средство',
    ],
];
